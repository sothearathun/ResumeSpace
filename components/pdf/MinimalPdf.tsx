...
export function MinimalPdf({ content, appearance }: { content: ResumeContent; appearance: ResumeAppearance }) {
    ...
    <View style={{ fontSize: m.fontSize, lineHeight: m.lineHeight, color: "#171717" }}>
      <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 12 }}>
        {contact.photoDataUrl && (
          <PdfAvatar name={contact.name} photoDataUrl={contact.photoDataUrl} shape={appearance.photoShape} size={scaledAvatarSize(85, appearance.photoSize)} accent={m.accent} />
        )}
        <View>
